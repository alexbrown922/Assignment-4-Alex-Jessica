...
# Note for Jess: Match this graph's colors to other graph
...
# Based on the student's t-test, there is no significant difference in mean sizes for lobster samples collected in 2012 and 2017 at Arroyo Quemado (p = `r round(arroyo_ttest$p.value,3)).
...
# Based on the student's t-test, there is no significant difference in mean sizes for lobster samples collected in 2012 and 2017 at Carpinteria (p = `r round(carp_ttest$p.value,3)).
...
# Based on the student's t-test, there is no significant difference in mean sizes for lobster samples collected in 2012 and 2017 at Isla Vista (p = `r round(ivee_ttest$p.value,3)).
...
# Based on the student's t-test, there is no significant difference in mean sizes for lobster samples collected in 2012 and 2017 at Naples (p = `r round(napl_ttest$p.value,3)).
...
# legal and non legal column for each site from counts column
...
lobster_counts <- lobster_abundance_tidy %>% 
  ...
  rename(SIZE >= "83")
  group_by(SITE) %>% 
  select(SITE, SIZE, SIZE >= "83", SIZE <= "82")
